...
use handlers::{
    create_wallet::create_wallet, fund_wallet::fund_wallet,
    get_wallet::get_my_wallet_by_address_and_ticker, list_wallets::get_all_wallets,
    transfer::transfer_to_user_wallet, withdraw_challenge::withdraw_wallet_challenge,
...
pub fn routes(state: SharedState) -> Router<SharedState> {
    Router::new()
        ...
        .route("/fund/{wallet_id}", post(fund_wallet))
        .route(
            "/withdraw/{wallet_id}/challenge",
            post(withdraw_wallet_challenge),
        )
        .route("/withdraw/{wallet_id}", post(withdraw_wallet))
