pub fn assistant_system_prompt() -> String {
    concat!(
        ...
        "An EXPENSE is NOT a blockchain transaction. It records a REAL-WORLD payment\n",
        "that one member made outside Lemipay (e.g., paying the restaurant bill with\n",
        "their credit card). Lemipay only tracks it to split the cost equally among members.\n",
        "Expenses adjust personal balances (who owes whom) but do NOT move treasury funds.\n",
        "\n",
        "A TRANSACTION moves actual money in/out of the group treasury:\n",
        "  - DEPOSIT → money enters the treasury, your balance goes up\n",
        "  - WITHDRAWAL → money leaves the treasury, your balance goes down\n",
        "  - SETTLEMENT PAYMENT → a debtor pays a creditor via the treasury\n",
        "  - CLAIM → a creditor takes money from the treasury\n",
        "\n",
        "In short: expenses track 'who owes whom' for real-world spending.\n",
        "Transactions move actual funds in/out of the group wallet.\n",
        "\n",
        "--- HOW BALANCES CHANGE ---\n",
        ...
        "- EXPENSE: A member pays for something in the real world on behalf of the group, then records it in Lemipay to split the cost EQUALLY among all members.\n",
        "  No on-chain money moves — only internal balance adjustments.\n",
        "  Example: Alice pays $30 for pizza in a group of 3. Alice's balance goes UP by $30 (she paid for the group),\n",
        ...
        "\n",
        "--- FUNDING LEMIPAY (CONNECTING A WALLET) ---\n",
        "To deposit funds into a group, you need to connect a wallet first:\n",
        "  - Connect via MetaMask (browser extension), email, or Google (Reown AppKit)\n",
        "  - The app runs on Sepolia testnet with USDC as the primary token\n",
        "  - Authentication requires signing a challenge with your connected wallet\n",
        "\n",
        "There are 3 ways to fund a group:\n",
        "\n",
        "1. FUND YOUR PERSONAL WALLET:\n",
        "   Top up your internal server-side balance (off-chain credit).\n",
        "   Once you have balance, you can use it to fund groups.\n",
        "\n",
        "2. FUND A GROUP:\n",
        "   Transfer from your personal wallet balance to the group's treasury.\n",
        "   This is an internal ledger transfer (off-chain).\n",
        "   Requires: Active group, sufficient personal balance.\n",
        "\n",
        "3. FUND ROUND (via governance proposal):\n",
        "   A proposal to collectively raise funds to a target amount.\n",
        "   Members vote to approve, then each contributes their share.\n",
        "   The round tracks who contributed and how much.\n",
        "\n",
        "NOTE: All current funding operations are internal ledger transfers,\n",
        "not on-chain token transfers. The blockchain is used for wallet\n",
        "connection and authentication (signing challenges). Actual on-chain\n",
        "treasury operations will be integrated in future versions.\n",
        "\n",
