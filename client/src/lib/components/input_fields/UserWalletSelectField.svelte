...
<div class="w-full">
    ...
    <div class="relative">
        ...
        >
            <option value="" disabled selected>
                {loading ? 'Buscando wallets compatibles...' : wallets.length === 0 ? 'Sin wallets compatibles' : 'Elegí una wallet de destino'}
            </option>
    ...
    {:else if wallets.length === 0 && !loading && !error}
        <p class="mt-1.5 text-xs text-gray-500">
            {currency_id ? 'No tenés wallets compatibles con la moneda de este grupo.' : 'No tenés wallets configuradas.'}
        </p>
    {:else if error}
